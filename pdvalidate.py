...
class pdvalid(gzqc):
	...
	self.dictionary = pd.read_excel(self.dictxls,sheet_name=None)
	...
	missingcolumnslist=[]
        ...
        for dkey in self.dictkeys[2:]:
        #for dkey in ['casetype']:#for test
            print '<--key-->' + dkey
            
	    for fname in self.files:
            #for fname in self.goodfiles:
                #I thought fnmatch is not case sensitive...
                if fnmatch.fnmatch(fname.lower(), ('*'+dkey.lower()+'*')):
                    print "%s matches pattern %s " % (fname,dkey)
                    dkeylist.append(dkey)
                    fnamelist.append(fname)
                    nrow=0
                    ncol=0
                    ncole=len(self.dictionary[dkey]['Column Physical Name'])
                    #ncole=3#for test
			    ...
			    missingcolumnslist.append(str(list(set(self.dictionary[dkey]['Column Physical Name'].str.strip()) - set(chunk.columns.str.strip()))))
                            extracolumnslist.append(str(list(set(chunk.columns.str.strip()) - set(self.dictionary[dkey]['Column Physical Name'].str.strip()))))
        ...
        self.summary=s_t
	print "Name patterns %s have no matching files found." %(list(set(self.dictkeys) - set(dkeylist)))

    def reportsummary(self):
        self.outputfile2=os.path.join(self.outputpath,'report_summary.txt')
        self.summary.to_csv(self.outputfile2,index=False,sep='|')
        print '%s%s'%('Report summary generated: ', self.outputfile2)
...
pdrawdata = pdvalid(datapath=config.get('input','datapath'),
...
print('---Count files---')
pdrawdata.countfiles()
print('---Format check---')
pdrawdata.formatcheck()
#print('---File shape---')
#pdrawdata.csvshape()
